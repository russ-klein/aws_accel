...
struct MaskPacket {
  enum {_issigned = issigned, _width = width};
  sc_lv<width> _mask;
};
...
// workaround for Questa 2020.X bug that causes chars and unsigned chars to crash
// when SC_REPORT_INFO is called. static cast them to int and unsigned int respectively
// before reporting. use the templated structs to have a catch all that doesn't cast when not chars
template<typename Tclass>
struct mc_format {
  typedef const Tclass type;
};
template<>
struct mc_format<char> {
  typedef const int type;
};
template<>
struct mc_format<unsigned char> {
...
struct mc_report_data {
  void report(const char* id, const Tclass& golden_value, const Tclass& dut_value) {
    typedef typename mc_format<Tclass>::type fmT;
    std::ostringstream msg;
    msg.str(""); msg << "          golden = " << static_cast<fmT>(golden_value);
    SC_REPORT_INFO(id, msg.str().c_str());
    msg.str(""); msg << "             dut = " << static_cast<fmT>(dut_value);
    SC_REPORT_INFO(id, msg.str().c_str());
  }
              ...
              const mgc_sysc_ver_array1D<Tclass,Tsize>& dut_value)
  {
    ...
    for (int i = LB; i <= UB; ++i) {
      ...
      if (golden_value[i] != dut_value[i]) {
        msg.str("###");
        msg << std::setw(6) << i << ' '
            << std::setw(20) << static_cast<fmT>(golden_value[i]) << ' '
            << std::setw(20) << static_cast<fmT>(dut_value[i]);
        SC_REPORT_INFO(id, msg.str().c_str());
      }
#else
        msg.str("");
...
public:  
  ...
  Tclass                             _dut;
  //_golden_sig and _dut_sig are used for SCVerify OutputCompare waveform view, do not remove
...
void mc_comparator<Tclass,TmaskPacket>::compare()
{
  while (true) {
    ...
    if (_golden._use_mask) {
      ...
      SC_REPORT_INFO(name(), _msg.str().c_str());
#endif
      enum { _elem_signed = TmaskPacket::_issigned,
             _elem_width  = TmaskPacket::_width,
             _mask_elems  = mc_container_type_traits<Tclass>::Tsize,
             _mask_width  = _elem_width * _mask_elems };

      sc_lv<_mask_width> _mask_lv, _temp_lv;
      // Build the mask logic vector
      for (int _elem_itr = 0; _elem_itr < _mask_elems; ++_elem_itr ) {
        _mask_lv.range((_elem_itr+1)*_elem_width-1, _elem_itr*_elem_width) = _golden._packet._mask;
      }
      //turn the data (Tclass) into a logic vector, mask the vector, and convert back for compare
      type_to_vector(_golden._data, _mask_width, _temp_lv);
      _temp_lv &= _mask_lv;
      vector_to_type(_temp_lv, _elem_signed, &_golden._data);

      //turn the dut (Tclass) into a logic vector, mask the vector, and convert back for compare
      type_to_vector(_dut, _mask_width, _temp_lv);
      _temp_lv &= _mask_lv;
      vector_to_type(_temp_lv, _elem_signed, &_dut);
