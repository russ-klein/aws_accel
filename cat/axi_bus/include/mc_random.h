...
// Check for macro definitions that will conflict with template parameter names
#if defined(T)
#define T 0
...
template <typename T, int W>
void mc_random_c_builtin_s<T, W>::operator()(T& v)
{
...
{
  enum { W = (sizeof(T) * CHAR_BIT) / 16 };
  mc_random_c_builtin_s<T,W>()(v);
}
...
inline void mc_random(sc_dt::sc_int_base& v) { mc_systemc_builtin_it(v, v.length()); }
inline void mc_random(sc_dt::sc_uint_base& v) { mc_systemc_builtin_it(v, v.length()); }
inline void mc_random(sc_dt::sc_signed& v) { mc_systemc_builtin_it(v, v.length()); }
inline void mc_random(sc_dt::sc_unsigned& v) { mc_systemc_builtin_it(v, v.length()); }
template <int Tlen>
inline void mc_random(sc_dt::sc_bv<Tlen>& v) { mc_systemc_builtin_it(v, Tlen); }
template <int Tlen>
inline void mc_random(sc_dt::sc_lv<Tlen>& v) { mc_systemc_builtin_it(v, Tlen); }
...
// Check for macro definitions that will conflict with template parameter names
#if defined(MCR_SMALL)
#define MCR_SMALL 0
#error The macro name 'MCR_SMALL' conflicts with a Template Parameter name.
#error The compiler should have produced a warning about redefinition of 'MCR_SMALL' giving the location of the previous definition.
#endif

template <class T, int W, bool MCR_SMALL>
...
template <class T, int W, bool MCR_SMALL>
void mc_random_ac_s<T,W,MCR_SMALL>::operator()(T& v)
{
  const ac_int<6> ac_16 = 16;
  ac_int<16> rv = rand();
  v = 0;
  v.set_slc(0, rv);
  for (int i = 16; i < W; i += 16) {
    v <<= ac_16;
    rv = rand();
    v.set_slc(0, rv);
  }
...
struct mc_random_ac_s<T,W,true>
{
  void operator()(T& v) { v.set_slc(0, ac_int<W>(rand())); }
};
