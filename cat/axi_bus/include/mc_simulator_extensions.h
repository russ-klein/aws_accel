...
#define SYSC_OBSERVE_FOREIGN_SIGNAL(signal,parent,path) {\
  std::string tmp0(parent); \
  std::string tmp1(#path); \
  std::string tmp2 = "/" + tmp0 + tmp1; \
  signal.observe_foreign_signal(tmp2.c_str()); }
#define CONTROL_FOREIGN_SIGNAL(signal,path) signal.control_foreign_signal(#path)
#define SYSC_CONTROL_FOREIGN_SIGNAL(signal,parent,path) {\
  std::string tmp0(parent); \
  std::string tmp1(#path); \
  std::string tmp2 = "/" + tmp0 + tmp1; \
  signal.control_foreign_signal(tmp2.c_str()); }
...
#define CONTROL_FOREIGN_SIGNAL(signal,path) {\
  std::string tmp(#path); \
  for (unsigned int i=0;i<tmp.length();i++) if (tmp[i]=='/') tmp[i]='.'; \
  signal.control_foreign_signal(tmp.substr(1).c_str()); }
...
#else
#if defined(CCS_DUT_VERILOG) || defined(CCS_DUT_SV)
...
#define CONTROL_FOREIGN_SIGNAL(signal,path) {\
  std::string tmp(#path); \
  for (unsigned int i=0;i<tmp.length();i++) if (tmp[i]=='/') tmp[i]='.'; \
  HDL_CONNECT_FN(signal,tmp.substr(1).c_str(),HDL_OUTPUT,0); }
...
#include "ccs_wrapper.h" // VCS generated header from HDL compile using -sc_model arg

#ifdef CCS_VCS_NAMESPACE
}
#define CCS_SCOPED_HDL CCS_VCS_NAMESPACE::ccs_wrapper
#else
#define CCS_SCOPED_HDL ccs_wrapper
#endif
