...
SC_MODULE(mc_stall_ctrl2) {
  ...
  unsigned int              rep_total;      // repetition countdown
  bool                      done;
    ...
    , next_stall(0)
  {
    ...
    // these options may be controlled by "flow package option set /SCVerify/STALL_<NAME>_COUNT #"
    // subtract by one since the state machine takes an extra state (clock cycle) in the state machine to complete 1 hold
    this->hold = (unsigned int) (ac_env::read_int("SCVerify_STALL_HOLD_COUNT", 3) - 1);
    this->hold = (this->hold > 0 ? this->hold : 1);
    this->rep_total = (unsigned int) (ac_env::read_int("SCVerify_STALL_REP_COUNT", 10));
    this->repetition = (this->rep_total > 0 ? this->rep_total : 1);
  }
  ...
  void generate() {
    ...
    if (rst.read() == 1) {
      if (!reset_done) {
        for (unsigned int i=0;i<BLOCKS;i++) { stall_out[i].write(sc_dt::Log_0); }
        stalled = 0;
        ldvd_pos = 0;
        intervalcnt = 0;
        repetition = rep_total;
        holdcnt = 0;
        reset_done = true;
        done = false;
      ...
      if (clk.read() == SC_LOGIC_1) {
        switch(cstate) {
          ...
          case MC_STALL_CTRL_STALL2: {
            if (--holdcnt == 0) cstate = MC_STALL_CTRL_STALL3; // hold stall for 'hold' clock cycles
            break;
          }
