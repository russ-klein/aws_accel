...
#if defined(N)
#define N 0
#error The macro name 'N' conflicts with an template parameter in this file.
#error The compiler should have produced a warning about redefinition of 'N' giving the location of the previous definition.
#endif
...
////////////////////////////////////////////////////////////////////////////////
// Helper struct to find the number of bits required for a value
// Similar to nbits in ac_int, which may not be available here
////////////////////////////////////////////////////////////////////////////////
template<unsigned N>
struct mc_nbits_rek {
  enum {
    Ndiv2 = N >> 1,
    val = 1 + mc_nbits_rek<Ndiv2>::val
  };
};
template<>
struct mc_nbits_rek<0> {
  enum { val = 0 };
};
template<unsigned N>
struct mc_nbits {
  enum { val = (N==0)? 1: mc_nbits_rek<N>::val };
};
...
struct mc_transactor_event_module : sc_module
{
  ...
  virtual int trans_latency() {
    if (this->get_attribute("TRANS_LATENCY"))
      return ((sc_attribute<int>*)(this->get_attribute("TRANS_LATENCY")))->value;
    return 0;
  }
};
...
public:
  ...
  virtual bool is_coupled() { return false; }
  virtual int trans_latency() { return 0; }
  ...
  // traits from derived rsc class
  virtual int get_width() { return 0; }
  virtual int get_rows() { return 0; }
  ...
  */
  ...
  virtual bool is_waiting(unsigned short iodir) { return false; }
...
template <class Tclass, int Telemwidth, bool Tsigned, int Tchan_factor = 1>
...
public:
  ...
  void configure_autowait(mc_wait_ctrl *new_wait_ctrl) {
    _auto_wait_ctrl = new_wait_ctrl;
    //need to preserve original stopat as a 'high water mark'
    //since later we add _ld_count to the auto_wait stopat
    _orig_stopat = _auto_wait_ctrl->stopat;
  }

  bool is_waiting(unsigned short iodir) {
    for (objblocks_iterator i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i) {
      if (((*i)->_mc_rsc_block)->is_waiting(iodir)) return true;
    }
    return false; 
  }

  bool atleast_one_autowait(int iodir) {
    for (objblocks_iterator i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i) {
      if (((*i)->_mc_rsc_block)->stall_count(iodir) > 0) return true;
    }
    return false;
  }
...
protected:
  ...
  virtual DataMode get_datamode() { return undef; }
  ...
  struct objblock {
    ...
    void transfer(access_fct fct, int brh, int brl, IT& val) {
      ...
      // A preserved struct channel may have interleaved field data when the word width is increased.
      // The assumption is that the rsc objblk metrics can be ignored because it's always a flat vector 
      //  of fields and has a 1:1 preserved var->rsc mapping.
      if ( Tchan_factor > 1 ) {
        // The indexing offsets and (virtual) row data need to be adjusted.
        sc_assert( brl==0 && brh==0 );
        const int row_width = int(ceil(double(_mc_rsc_block->get_width()) / Tchan_factor));
        for (int i=0; i < Tchan_factor; ++i) {
          const int rsc_idx = i * row_width + _cl;
          const int var_idx = i * _cc;
          (_mc_rsc_block->*fct)(0, rsc_idx, _cc, val, var_idx);
        }
        return;
      }
      for (; brl <= brh; ++brl) {
    ...
    int                        _trans; // transaction number of current value (may include "empty" read transactions)
    int                        _rdcnt; // completed read transactions
    int                        _wrcnt; // completed write transactions
  };
  ...
  mc_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit, int auto_wait_limit=0, mc_wait_ctrl *auto_wait_ctrl=0)
    ...
    , _auto_wait_ctrl(auto_wait_ctrl)
    , _orig_stopat(0)
    , _clk("_clk")
    , _sw_rst("_sw_rst")
  {}
  ...
  virtual bool check_disabled_input(objblock* blk) { return false; }
  ...
  virtual void send_value_to_tb(bool) {
    sc_assert(!_values.empty());
    _values.pop_front();
    ++_trans_begin;
  }
...
template <class Tclass, int Telemwidth, bool Tsigned, int Tchan_factor>
void mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>::exchange_value(int blk_id, bool waiting)
{
//  sc_assert(_phase ? _clk.posedge() : _clk.negedge()); // must be called at active clock edge
  ...
  if (blk->_trans == 0) {
    ...
    bool cdi = check_disabled_input(blk); // re-enable input if fifo has data
    ...
    if (waiting && !cdi) {
      ...
      if ((blk->_triosy->read() == SC_LOGIC_1) || _pending_triosy) {  // only valid at active clock edge
        if (blk->_mc_rsc_block->bus_transfer_required()) {
          ...
          bool send_output = this->get_datamode() == data_out || !cdi; // alt: !(data_in_out && cdi)
          if ( send_output ) {
            receive_value_from_objblock(blk);       // grab old output value first ...
          }
          if (blk->_trans++ == _trans_begin) {
            ...
            for (objblocks_iterator i = _objblocks.begin(); send_value && i != _objblocks.end(); ++i)
              if ((*i)->_trans == _trans_begin)
                send_value = false;                // current value is still needed by other block
            if (send_value) {
              // Remove the used vector from the queue. Outputs first convert the vector to c++ type and push to tb
              send_value_to_tb(send_output);
              _pending_triosy = false;
            }
          }
          blk->_mc_rsc_block->reset_stream();      // reset stream pointers
          send_value_to_objblock(blk);             // ... and place new input value afterwards
        }
      }
    }
  }
...
template <class Tclass, int Telemwidth, bool Tsigned, int Tchan_factor>
void mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>::register_block(mc_rsc_block_base*   blk,
                                             const char*          name,
                                             sc_signal<sc_logic>& triosy,
                                             int first, int last, int step)
{
...
public:
  ...
  void set_disable_on_empty(const bool val) { _disable_on_empty = val; }
  bool get_disable_on_empty() { return _disable_on_empty; }

  bool is_input_disabled() {
    return (_disable_on_empty && _input_disabled && in_fifo->used() == 0);
  }
  bool flush_disabled_input() {
    if ( is_input_disabled() ) {
      set_disable_on_empty(false);
...
protected:
  ...
  typedef typename base::DataMode DataMode;

  virtual DataMode get_datamode() { return base::data_in; }
  ...
  virtual void initiate_driving_value_adjustments(int blk_id, int RH, int BaseAddr, int CH, int BaseBit) {
    ...
    if (!this->sw_reset_active() && !this->_empty && blk->_triosy->read() == SC_LOGIC_1
      ...
      if (blk->_rdcnt < _ld_count || in_fifo->used() > 0) {
  ...
  virtual void send_value_to_objblock(objblock* blk) {
    ...
    if (!this->_empty) {
      ...
      if (this->sw_reset_active()) blk->_rdcnt = 1;
      else blk->_rdcnt++;
      ...
      if (this->log_event(MC_TRANSACTOR_READ)) {
        ...
        msg << "MC_TRANSACTOR_READ - sending value #" << blk->_rdcnt << " to '" << blk->_name << "' value: " << t << " @ " << sc_time_stamp();
        SC_REPORT_INFO(this->name(), msg.str().c_str());
      }
    }
  }

  virtual bool check_disabled_input(objblock* blk) {
  ...
  virtual void receive_value_from_tb() {
    ...
    if (!in_fifo->nb_get(t)) {
      ...
      if (_disable_on_empty && !_input_disabled) {
      ...
      if (!this->_empty && this->log_event(MC_TRANSACTOR_EMPTY)) {
        ...
        if (_disable_on_empty) {
           msg << "MC_TRANSACTOR_EMPTY - fifo is empty (disabling input) @ " << sc_time_stamp();
        } else {
      ...
      if (!_input_disabled && this->_empty && this->log_event(MC_TRANSACTOR_UNDERFLOW)) {
        std::ostringstream msg;
        msg << "MC_TRANSACTOR_UNDERFLOW - fifo underflow detected (garbage has been read) @ " << sc_time_stamp();
        SC_REPORT_WARNING(this->name(), msg.str().c_str());
      }
      if (_input_disabled && this->_empty && this->log_event(MC_TRANSACTOR_EMPTY)) {
        std::ostringstream msg;
        msg << "MC_TRANSACTOR_EMPTY - fifo no longer empty (input re-enabled) @ " << sc_time_stamp();
        SC_REPORT_INFO(this->name(), msg.str().c_str());
      }
      if (_input_disabled) {
          _input_disabled = false;
     ...
     if (this->_auto_wait_limit == 0) {
      //do user testbench supplied wait controlling
      ...
      if (in_wait_ctrl_fifo->nb_peek(wait_ctrl)) {
         if (wait_ctrl.iteration == _ld_count) {
            ...
            if (this->log_event(MC_TRANSACTOR_WAIT) && !wait_ctrl.quiet) {
               std::ostringstream msg;
               msg << "(input) MC_TRANSACTOR_WAIT - Initiating user specified wait: " << wait_ctrl << " @ " << sc_time_stamp();
               SC_REPORT_INFO(this->name(), msg.str().c_str());
            }
            if (user_wait_cycles) {
               for (obi i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i)
      ...
      this->_auto_wait_ctrl->iteration = _ld_count;
      this->_auto_wait_ctrl->stopat = this->_orig_stopat + _ld_count;
      for (obi i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i) {
        bool messaged = false;
        if (1
            ...
            && ( ((*i)->_mc_rsc_block)->io_request(0) || _ld_count == 0 ) //special case to capture the first transaction at zero time
        ) {
          ((*i)->_mc_rsc_block)->wait_cycles(*this->_auto_wait_ctrl,0); // iodir=0 means input
          ((*i)->_mc_rsc_block)->incr_stall_count(0); // iodir=0 means input
          if (!messaged && this->log_event(MC_TRANSACTOR_WAIT)) {
            std::ostringstream msg;
            msg << "(input) MC_TRANSACTOR_WAIT - Initiating automatic I/O wait: " << *this->_auto_wait_ctrl << " @ " << sc_time_stamp();
            SC_REPORT_INFO(this->name(), msg.str().c_str());
          }
          messaged = true;
        }
...
template <class Tclass, int Telemwidth, bool Tsigned, int Tchan_factor=1>
class mc_output_transactor : virtual public mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>
{
public:
  ...
  mc_output_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit, int auto_wait_limit=0)
    : mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>(name,BaseAddr,WordWidth,BaseBit,auto_wait_limit)
    , out_fifo("out_fifo")
    , _dmp_count(0)
  {}

protected:
  typedef mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>        base;
  typedef typename base::objblock           objblock;
  typedef typename base::IT              IT;
  typedef typename base::objblocks_iterator obi;
  typedef typename base::DataMode DataMode;

  virtual DataMode get_datamode() { return base::data_out; }
  ...
  void receive_value_from_objblock(objblock* blk) {
    ...
    if (this->log_event(MC_TRANSACTOR_WRITE)) {
      ...
      vector_to_type(v, Tsigned, &t);
      std::ostringstream msg;
      msg << "MC_TRANSACTOR_WRITE - received value #" << blk->_wrcnt << " from '" << blk->_name << "' : " << t << " @ " << sc_time_stamp();
      SC_REPORT_INFO(this->name(), msg.str().c_str());
    }
  }

  virtual void send_value_to_tb(bool send_output) {
    if ( send_output ) {
      if (this->_auto_wait_limit == 0) {
        //do user testbench supplied wait controlling
        //Modified wait controls
        // This function is not called before the first value from the dut is available.
        ...
        // Automatic random stall insertion based on active I/O
        this->_auto_wait_ctrl->iteration = _dmp_count;
        this->_auto_wait_ctrl->stopat = this->_orig_stopat + _dmp_count;
        for (obi i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i) {
          bool messaged = false;
          if (1
              && !((*i)->_mc_rsc_block)->get_is_disabled()
            ...
            ((*i)->_mc_rsc_block)->wait_cycles(*this->_auto_wait_ctrl,1); // iodir=1 means output
            ((*i)->_mc_rsc_block)->incr_stall_count(1); // iodir=1 means output
            if (!messaged && this->log_event(MC_TRANSACTOR_WAIT)) {
              std::ostringstream msg;
              msg << "(output) MC_TRANSACTOR_WAIT - Initiating automatic I/O wait: " << *this->_auto_wait_ctrl << " @ " << sc_time_stamp();
              SC_REPORT_INFO(this->name(), msg.str().c_str());
            }
            messaged = true;
          }
      ...
      if (!out_fifo->nb_put(t)) {
        ...
        if (this->log_event(MC_TRANSACTOR_DUMP)) {
          std::ostringstream msg;
          msg << "MC_TRANSACTOR_DUMP - fifo dumped value #" << _dmp_count << " : " << t << " @ " << sc_time_stamp();
          SC_REPORT_INFO(this->name(), msg.str().c_str());
        }
        if (this->get_attribute("MC_TRANSACTOR_COUNT")) {
...
struct mc_inout_transactor : mc_input_transactor<Tclass,Telemwidth,Tsigned>,  mc_output_transactor<Tclass,Telemwidth,Tsigned>
{
  mc_inout_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit, int auto_wait_limit=0)
    : mc_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit,auto_wait_limit)
    , mc_input_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit,false,auto_wait_limit)
  ...
  typedef mc_transactor<Tclass,Telemwidth,Tsigned> base;
  typedef typename base::DataMode DataMode;
  virtual DataMode get_datamode() { return base::data_in_out; }
};
...
template <class Tclass, int Telemwidth, bool Tsigned, int Tchan_factor=1>
class mc_channel_input_transactor : virtual public mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>
{
public:
  ...
  mc_channel_input_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit, int min_trans_latency, bool disable_on_empty=false, int auto_wait_limit=0)
    : mc_transactor<Tclass,Telemwidth,Tsigned,Tchan_factor>(name,BaseAddr,WordWidth,BaseBit,auto_wait_limit)
    , in_fifo("in_fifo")
    , _ld_count(0)
    , _disable_on_empty(disable_on_empty)
    , _input_disabled(false)
    , _input_disabled_ignore_first_check(true)
    , _input_disabled_required(false)
    , _input_disabled_hold_cnt(0)
    , _min_trans_latency(min_trans_latency)
  {
    sizelz_count = 0;
    _input_fifo_size = 0;
  }
...
protected:
  ...
  virtual void send_value_to_objblock(objblock* blk) {
    ...
    if (!this->_empty) {
      ...
      if (this->log_event(MC_TRANSACTOR_READ)) {
        ...
        vector_to_type(v, Tsigned, &t);
        std::ostringstream msg;
        msg << "MC_TRANSACTOR_READ - sending value #" << blk->_rdcnt << " to '" << blk->_name << "' : " << t << " @ " << sc_time_stamp();
        SC_REPORT_INFO(this->name(), msg.str().c_str());
      }
    }
  }
  ...
  virtual bool check_disabled_input(objblock* blk) {
    mc_wait_ctrl wait_ctrl;
    bool empty_queued = in_wait_ctrl_fifo->nb_peek(wait_ctrl) && wait_ctrl.cycles == 0;
    if (_input_disabled_ignore_first_check) {
      _input_disabled_ignore_first_check = false;
      if (!empty_queued) return false;
    }
    if (empty_queued && wait_ctrl.iteration == _ld_count) {
      _input_disabled_required = _disable_on_empty && !_input_disabled;
      _input_disabled_hold_cnt = _disable_on_empty? wait_ctrl.hold_count: 0;
      in_wait_ctrl_fifo->nb_get(wait_ctrl); // pop
      wait_ctrl.clear();
      int trans_latency = blk->_mc_rsc_block->trans_latency();
      if (trans_latency > _min_trans_latency) {
        if (_ld_count == 0) {
          // If the first transaction is empty then it needs to be delayed by its relative latency.
          _input_disabled_hold_cnt += trans_latency - _min_trans_latency;
        }
        return false;
      }
      if (!blk->_mc_rsc_block->io_request(0)) {
        return false;
      }
    }

    if (_disable_on_empty && _input_disabled && (in_fifo->used()>0)) {
      if (_input_disabled_hold_cnt) {
        _input_disabled_hold_cnt--;
        return false;
      } 
      if (0&& !blk->_mc_rsc_block->io_request(0)) {
        // If an empty transaction is expected where the TB attempts to read then waiting for rdy could be used.
        // Conversely, if the TB is not reading because IO is conditional then rdy should not be checked.
        // This would help for II>1 and conditional rates via waits etc.
        // Currently there is no way to know if the TB is reading or not, so the hold counter is needed.
        return false;
      }
      _input_disabled_required = false;
      for (obi i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i)
  ...
  virtual void receive_value_from_tb() {
    ...
    if (_input_disabled_required || !in_fifo->nb_get(t)) {
      if (_disable_on_empty && !_input_disabled) {
        _input_disabled = true;
        for (obi i = this->_objblocks.begin(); i != this->_objblocks.end(); ++i)
          ((*i)->_mc_rsc_block)->set_is_disabled(true);
     ...
     if (this->_auto_wait_limit == 0) {
      ...
      if (in_wait_ctrl_fifo->nb_peek(wait_ctrl)) {
         if (wait_ctrl.stopat <= _ld_count) {
            in_wait_ctrl_fifo->nb_get(wait_ctrl); // pop
            if (this->log_event(MC_TRANSACTOR_WAIT) && !wait_ctrl.quiet && wait_ctrl.cycles > 0) {
               std::ostringstream msg;
               msg << "(input) MC_TRANSACTOR_WAIT - Clearing user specified wait: " << wait_ctrl << " @ " << sc_time_stamp();
               SC_REPORT_INFO(this->name(), msg.str().c_str());
            }
            // clear out wait_ctrl
            wait_ctrl.clear();
...
private:
  ...
  bool _disable_on_empty;
  bool _input_disabled;
  bool _input_disabled_ignore_first_check; //after reset
  bool _input_disabled_required;
  int  _input_disabled_hold_cnt;
  int  _min_trans_latency;
};
...
public:
  ...
  virtual int get_width() { return Twidth; }
  virtual int get_rows() { return Trows; }
  ...
  virtual void set_is_disabled(bool is_disabled) {
    this->_is_disabled = is_disabled; _wait_cycles_changed[0].notify();
  }
  ...
  // Access the TRANS_LATENCY attribute if available
  virtual int trans_latency() {
    if (this->get_attribute("TRANS_LATENCY"))
      return ((sc_attribute<int>*)(this->get_attribute("TRANS_LATENCY")))->value;
    return 0;
  ...
  bool is_waiting(unsigned short iodir)   { return (_wait_cycles_cntr[iodir] > 0 && wait_signal_active(iodir)); }
  void wait_controller(unsigned short iodir) {
    if (get_is_disabled()) {
      ...
      if (_wait_cycles_cntr[iodir] <= 0) {
        ...
        if (!_wait_io_request[iodir] && this->log_event(MC_TRANSACTOR_WAIT) && !this->is_coupled() && !_wait_ctrl[iodir].quiet) {
          std::ostringstream msg;
...
////////////////////////////////////////////////////////////////////////////////
// SystemC module to generate global sync signals for SCVerify top
...
////////////////////////////////////////////////////////////////////////////////
// SystemC module to generate sync signal for transactors and resource blocks.
// Necessary for interfaces without feedback from the Catapult process: 
//  ac_channel interfaces that don't have an lz handshake (in.rdy, out.vld)
//  or interface vars that do not use transaction done signals.
////////////////////////////////////////////////////////////////////////////////
class scverify_top;

SC_MODULE(iosync_generator) {
  SC_HAS_PROCESS(iosync_generator);

  sc_in<bool>      clk;
  sc_in<sc_logic>  rst;
  sc_in<sc_logic>  core_wen;
  sc_out<sc_logic> iosync;
  sc_port<tlm::tlm_get_peek_if<mc_sync_ctrl> > sync_ctrl_fifo;
  ...
  : sc_module(name),
    ...
    core_wen("core_wen"),
    iosync("iosync"),
    sync_ctrl_fifo("sync_ctrl_fifo"),
    gen_enable("gen_enable"),
    clk_phase(_clk_phase),
    en_phase(_en_phase),
    iodir(_iodir),
    start_latency(_start_latency),
    init_interval(_init_interval)
  {
    start = true;
    iosync_cnt = 0;
    ...
    add_ramp_down = 0;
    wait_msg_toggle = true;
    wait_msg_enable = true;
    #ifdef MC_DEFAULT_TRANSACTOR_LOG
    wait_msg_enable = MC_DEFAULT_TRANSACTOR_LOG & MC_TRANSACTOR_WAIT;
    #endif

    SC_METHOD(generator);
    sensitive << (clk_phase? clk.pos(): clk.neg());
    if ( is_input() ) sensitive << gen_enable;
    dont_initialize();

    SC_METHOD(gen_enable_drv);
    sensitive << delta_trigger[1];
    dont_initialize();

    SC_METHOD(gen_enable_drv1);
    sensitive << delta_trigger[0];
    dont_initialize();

    SC_METHOD(gen_enable_drv0);
    sensitive << core_wen;
    dont_initialize();
  }

  // Generate an IO sync signal for updating transactor resources
  void generator() {
    if ( rst.read() == SC_LOGIC_1 ) {
      start = true;
      iosync_cnt = 0;
      update_user_sync();
      iosync.write(SC_LOGIC_0); 
    } else if ( !pause_on_stall || gen_enable.read() == (en_phase? SC_LOGIC_1: SC_LOGIC_0) ) {
      iosync_cnt++;
      iosync_cnt_dbg.write(iosync_cnt);
      update_user_sync();
      if ( ramp_down_cyc > 0 && sample_cnt == capture_cnt )
        add_ramp_down = ramp_down_cyc; 
      if ( (start && (iosync_cnt == start_latency || start_latency == 0)) 
        || (!start && iosync_cnt == (init_interval + add_ramp_down)) )
      {
        start = false;
        add_ramp_down = 0;
        iosync_cnt = 0;
        sample_cnt++;
        update_sync_controls();
        iosync.write(SC_LOGIC_1); 
        #ifdef CCS_SCVERIFY_DEBUG
        wait_msg_toggle = wait_msg_enable;
        std::cout << "*DBG: " << this->name() << ": (" << sample_cnt << ") iosync active @" << sc_time_stamp() << std::endl;
        #endif
      } else {
        iosync.write(SC_LOGIC_0); 
      }
      ...
      if ( wait_msg_toggle ) {
        std::ostringstream msg;
        msg << "The syncronization rate controller is disabled while process is stalled";
        #ifdef CCS_SCVERIFY_DEBUG
        msg << " @ " << sc_time_stamp();
        #endif
        SC_REPORT_INFO(this->name(), msg.str().c_str());
        wait_msg_toggle = false;
      }
      iosync.write(SC_LOGIC_0);
    }
  }

  bool is_input() { return !iodir; }
  bool is_output() { return iodir; }

  protected:
  friend class scverify_top;
  void initialize_sync_controls (bool _pause_on_stall, int _start_latency, int _init_interval) {
    pause_on_stall = _pause_on_stall;
    start_latency  = _start_latency;
    init_interval  = _init_interval;
  }
  ...
  enum sync_mode {
    nop = 0,
    init_sync,   // Update sync controls during reset
    local_sync,  // Local capture_cnt in use, sync controls update on next iosync
    fifo_sync    // Lagging update from TB, update sync controls immediately
  };

  // User controls from TB are updated when the captured element count from the TB matches 
  //   the local sample counter, which tracks the number of iosync updates sent to the transactor
  void update_user_sync() {
    mc_sync_ctrl tmp_sync;
    if ( capture_cnt < capture_cnt_next )
      capture_cnt = capture_cnt_next;
    if ( sync_ctrl_fifo->nb_peek(tmp_sync) ) {
      sync_mode mode = (start && capture_cnt == 0)? init_sync:
                       (capture_cnt == sample_cnt)? local_sync:
                       (tmp_sync.capture_cnt == sample_cnt)? fifo_sync: nop;

      if ( mode != nop ) {
        #ifdef CCS_SCVERIFY_DEBUG
        std::cout << "*DBG: " << this->name() << " update_user_sync: " << "sync_mode=" << mode << ", next capture_cnt=" << tmp_sync.capture_cnt 
                  << " (" << capture_cnt << ":" << sample_cnt <<")" << " @ " << sc_time_stamp() << std::endl;
        #endif
        sync_ctrl_fifo->nb_get(user_sync); //pop
        capture_cnt_next = user_sync.capture_cnt;
        if ( is_output() || mode != local_sync ) {
          update_sync_controls();
        }
      }
    }
  ...
  void update_sync_controls () {
    bool update_sync = false;
    if ( user_sync.start_latency > 0 ) {
      update_sync = true;
      start_latency = user_sync.start_latency;
    }
    if ( user_sync.init_interval > 0 ) {
      update_sync = true;
      init_interval = user_sync.init_interval;
    }
    if ( user_sync.ramp_down_cyc >= 0 ) {
      update_sync = true;
      ramp_down_cyc = user_sync.ramp_down_cyc;
    }
    if ( user_sync.pause_on_stall != pause_on_stall ) {
      update_sync = true;
      pause_on_stall = user_sync.pause_on_stall;
    }
    if (update_sync) {
      #ifdef CCS_SCVERIFY_DEBUG
      std::cout << "*DBG: " << this->name() << " update_sync_controls: start_latency=" << start_latency 
                << ", init_interval=" << init_interval << " ramp_down_cyc=" << ramp_down_cyc << " pause_on_stall=" << pause_on_stall
                << " (" << capture_cnt << ":" << sample_cnt <<")" << " @ " << sc_time_stamp() << std::endl;
      #endif
      user_sync.clear();
    }
  ...
  // gen_enable is used as an interrupt to disable the rate counter when the IO's process is stalled (core_wen signal)
  // The delta_triggers are used because of delta cycle glitches on core_wen. Each assignment results in a delta cycle
  //   so this is used to move past the glitch in the event queue _and_ stay in the same time step which is preferable. 
  sc_signal<sc_logic> gen_enable;
  sc_signal<sc_logic> delta_trigger[2];

  void gen_enable_drv() {
    //next_trigger(1,SC_PS); // only works if time is within sim resolution (can't use SC_ZERO_TIME for delta cycle steps)
    #ifdef CCS_SCVERIFY_DEBUG
    if (core_wen.read() != delta_trigger[1] ) {
      std::cout << "*DBG: " << this->name() << ": core_wen glich @ " << sc_time_stamp() << std::endl;
    }
    #endif
    gen_enable = core_wen.read();
  }
  void gen_enable_drv1() {
    delta_trigger[1] = delta_trigger[0].read();
  }
  void gen_enable_drv0() {
    delta_trigger[0] = core_wen.read();
  }

  bool clk_phase;
  bool en_phase;
  bool start;
  bool iodir;                 // Output == true
  int  start_latency;         // Initial latency after reset
  int  init_interval;         // Throughput sync control rate
  int  ramp_down_cyc;         // Cycles at the end of a non-piplined process loop
  bool pause_on_stall;        // Use the leaf reader/writer block core_wen signal to stall generator
  int  iosync_cnt;            // IO sync control rate counter
  int  sample_cnt;            // Track local transaction counts from iosync updates in generator 
  int  capture_cnt;           // Track transaction capture counts from TB for synchronizing updates from user 
  int  capture_cnt_next;
  int  add_ramp_down;
  mc_sync_ctrl user_sync;     // Updates from user's TB sync controls, also sends TB capture count
  bool wait_msg_enable;
  bool wait_msg_toggle;

  sc_signal<int> iosync_cnt_dbg;  // Debug signal for sync rate counter display in wave window
};
