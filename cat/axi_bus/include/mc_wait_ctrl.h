...
struct mc_wait_ctrl {
  ...
  mc_wait_ctrl(
    ...
    int n,
    bool chan=false
  ) : iteration(i), stopat(s), cycles(c), interval(n), mode(m), elements(e),
      ischannel(chan), hold_count(-1), quiet(false)
  {}

  void clear() { iteration=0; stopat=0; cycles=0; mode=UNIFORM; elements=INITIAL; interval=0; hold_count=-1; }
  bool is_set() { return (cycles > 0 || hold_count >= 0); }
  ...
  int                   hold_count; // hold count cycles for disabled input (ac_channel non-blocking reads)
  bool                  quiet;      // true only if enabled by user directly (not cleared)
};
