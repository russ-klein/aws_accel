...
int main() {
  ...
  r = fpga_pci_poke(handle, 0, 3);
  printf("fpga_poke @0 => %d \n", r);
  if (r) return r;

  fpga_pci_poke(handle, 4, 4);
  printf("fpga_poke @4 => %d \n", r);
  if (r) return r;

  fpga_pci_poke(handle, 8, 5);
  printf("fpga_poke @8 => %d \n", r);
  if (r) return r;

  fpga_pci_peek(handle, 12, &value);
  printf("fpga_peek @12 => %d \n", r);
  if (r) return r;
